...
func (l *Laptop) Find(ctx context.Context,brand string,model string) (error) {
	if l.Client == nil {
	...
	err := l.Client.Database(l.Dbname).Collection(l.Collection).FindOne(ctx, bson.D{{"model",model},{"brand",brand}}).Decode(&document)
	if err != nil {
		return err
	}
